...
from ace_edsl.edsl.core.vector_ops import vec_add, vec_mul, vec_roll, vec_slice
...
def baseline_gemm_vector_kernel(
    ...
    if prepared.input_duplications > 1:
        ...
        input_dup = vec_add(
            ...
            vec_roll(
                ...
                duplicate_rotations.candidates,
            ),
        )
    ...
    for iv in range_dynamic(
        ...
        rolled_input = vec_roll(
            input_dup, iv, gemm_rotations.candidates
        )
        sliced_weight = vec_slice(weight, iv, weight_slice.width)
        result = vec_add(result, vec_mul(rolled_input, sliced_weight))
    ...
    if prepared.reductions:
        ...
        for iv in range_dynamic(
            ...
            result = vec_add(
                result,
                vec_roll(result, shift, reduction_rotations.candidates),
            )

    biased = vec_add(result, bias)
    container.new_stid(result_name, biased.value)
    if prepared.mask.policy != "none":
        masked = vec_mul(result, constants["mask"])
