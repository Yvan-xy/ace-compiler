...
void VECTOR_KERNEL_LOWERING_REGISTRY::Clear() { _selectors.clear(); }
...
Try_materialize_registered_vector_kernel(
  ...
  AIR_ASSERT(spec._result_type != Null_ptr);
  AIR_ASSERT(&spec._result_type->Glob_scope() == destination);
  ...
  FUNC_SCOPE* helper_scope =
      Materialize_helper(*destination, spec, source_node->Spos());
  ...
  NODE_PTR replacement =
      caller_cntr->New_ldp(result_preg, source_node->Spos());
