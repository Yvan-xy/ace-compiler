...
lock_value CUDA_IMAGE >/dev/null
lock_value CUDA_IMAGE_CONFIG >/dev/null
...
RUNNER_SOURCE="$(
  git -C "${REPO_ROOT}" show \
    "${ACE_COMMIT}:tools/phantom_gpu/run_retained_ckks_correctness.sh"
)"
for forbidden in nvidia-smi compute-sanitizer cuda-memcheck; do
  if grep -Fq "${forbidden}" <<<"${RUNNER_SOURCE}"; then
    echo "selected retained host runner contains forbidden GPU execution tooling: ${forbidden}" >&2
    exit 1
  fi
done
if ! grep -Fq '"gpu_executables_were_run": False' <<<"${RUNNER_SOURCE}"; then
  echo "selected retained host runner lacks the no-GPU-execution attestation" >&2
  exit 1
fi
...
payload = {
    "schema_version": "ace.phantom.retained-host-freeze-payload/1.0.0",
    ...
    "fixture_lifecycle_modes": {
        "provisional": "candidate-binding-only",
        "formal": "checked-bound-frozen-evidence",
    },
...
echo "audited retained host-freeze source payload: ${OUTPUT}"
